...
View(psych::describe(numeric_data))
```
...
subset_school_meas_of_center_long <- subset_school_statistics |>
  pivot_longer(cols = everything(),
               names_to = c("Feature", "Statistic"),
                            names_sep = "_",
               values_to = "value")
View(subset_school_meas_of_center_long)
...
school_sex_meas_of_center_long <- school_sex_statistics |>
  pivot_longer(cols = -c("school","sex"),
               names_to = c("Feature", "Statistic"),
                            names_sep = "_",
               values_to = "value")
View(school_sex_meas_of_center_long)
```
...
kernel_plot <- ggplot(combine_data, aes(x = age, color = school, linetype = as.factor(failures))) +
...
```{r}
# Kernel Density Plot
kernel_plot <- ggplot(combine_data, aes(x = age, color = sex, linetype = as.factor(failures))) +
  geom_density(alpha = 0.5, aes(fill = as.factor(failures))) +
  theme_minimal() +
  labs(
    title = "Kernel Density Plot of Age by Reason and Failures",
